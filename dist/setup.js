...
import { sleep, readJson, writeJson } from './helpers/util.js';
import { keypairFromSecretJson, loadAnchorProgram, sendTransactionWithRetryWithKeypair } from './helpers/solana.js';
import { decodeMetadata, getMetadata, TOKEN_ENTANGLEMENT_PROGRAM_ID, signMetadataInstruction, TOKEN_METADATA_PROGRAM_ID } from './helpers/metaplex.js';
...
async function getJuiceBoxTrashHolders(connection) {
...
async function createAllEntangledImages(toDrawPath, outputPath) {
...
async function createEntangledNFTs(amount, connection, keypair) {
    let entangledMints = [];
...
async function signAllEntangledMetadata(metadatas, connection) {
}
...
export async function arweaveUpload(walletKeyPair, connection, env, image, manifestBuffer) {
    ...
    const tx = await sendTransactionWithRetryWithKeypair(connection, walletKeyPair, instructions, [], 'confirmed');
    ...
    const imageFile = result.messages?.find((m) => m.filename === image);
...
async function upload(data) {
    return await (await fetch(ARWEAVE_UPLOAD_ENDPOINT, {
        method: 'POST',
        // @ts-ignore
        body: data,
    })).json();
}
async function entangleSoldierPanda(keypair) {
