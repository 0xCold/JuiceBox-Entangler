...
const TOKEN_ENTANGLEMENT_PROGRAM_ID = new PublicKey(
...
async function sendTransactionWithRetryWithKeypair(
    connection: Connection,
    wallet: Keypair,
...
function signMetadataInstruction(
    metadata: PublicKey,
    creator: PublicKey,
): TransactionInstruction {
...
async function mintRemaining(keypairPath: string, rpcUrl: string, mintsPath: string, urisPath: string) {
...
async function signAll(keypairPath: string, rpcUrl: string, mintsPath: string) {
    const keypair = Keypair.fromSecretKey(
        Uint8Array.from(JSON.parse(fs.readFileSync(keypairPath).toString()))
    );
    const connection = new Connection(rpcUrl);
...
async function entangleAll(keypairPath: string, rpcUrl: string, mintsPath: string) {
    const keypair = Keypair.fromSecretKey(
        Uint8Array.from(JSON.parse(fs.readFileSync(keypairPath).toString()))
    );
    const connection = new Connection(rpcUrl);
...
async function main(keypairPath: string, rpcUrl: string){
